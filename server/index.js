...
import { buildApiResponse, getSchemeWithFallback, refreshAllTracked, loadCachePayload } from './services/navService.js'
...
/** Cache-only (fast). Live refresh runs on startup (if empty), daily cron, or manual script. */
...
app.get('/api/nav/:schemeCode', async (req, res) => {
  ...
  try {
    ...
    if (!out.data) {
      return res.status(404).json({ detail: out.error ?? 'Not found', usingCachedData: out.usingCachedData })
    }
    return res.json({
    ...
    return res.status(502).json({ detail: String(e.message ?? e), usingCachedData: true })
  }
})

/**
 * Mounted at `/api`: Express passes stripped paths (e.g. `/health`) to the proxy.
 * Rewrite back to FastAPI’s `/api/*` routes.
 */
app.use(
  '/api',
...
async function main() {
  try {
    ...
    if (!p.lastSyncedAt || Object.keys(p.entries).length === 0) {
      console.info('[nav] Empty cache — refreshing tracked funds…')
      await refreshAllTracked()
    }
  } catch (e) {
    console.warn('[nav] Startup refresh failed (will use cache on requests):', e.message)
  }

  app.listen(PORT, () => {
    console.info(`API gateway listening on :${PORT} → FastAPI ${FASTAPI_TARGET}`)
    startNavRefreshJob()
  })
}
